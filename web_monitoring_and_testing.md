...
* **`18030` (k6 Prometheus (Native Histograms)):** This maps the manual `k6` frontend browser scripts. It populates dashboards focused on Web Vitals metrics, page rendering behaviors, and click-to-load interaction timeframes, as well as showing loadtest results when the command for loadtesting is run with "K6_PROMETHEUS_RW_TREND_AS_NATIVE_HISTOGRAM=true k6 run --out experimental-prometheus-rw=http://localhost:9090/api/v1/write --vus 5 --duration 10s loadtest.js".
