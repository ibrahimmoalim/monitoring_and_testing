...
* **For Traces (Tempo):** To see traces, your applications themselves need to be written to send trace data (using a library like OpenTelemetry) follow this [setup](https://github.com/ibrahimmoalim/prometheus_and_grafana/blob/main/distributed_tracing.md), now go to **Explore** tab in Grafana, change the data source at the top left to **Tempo**, Choose Search in `Query type`, now under `Service Name` you should see the name you chose in the long run command e.g `WalletApp`, selsct it and hit `Run query` in the the top right (the blue button), explanation of the query result is in [setup](https://github.com/ibrahimmoalim/prometheus_and_grafana/blob/main/distributed_tracing.md#explaining-the-tempo-query-in-grafana).
